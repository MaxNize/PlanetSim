...
export function StateDisplay({
  ...
  const { mode, currentState, fps, frameTimeMs, fpsStatus } = useSimulationContext();
  const { t } = useI18n();
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
        ...
        <div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: '6px' }}>
            <div style={{ fontSize: '16px', fontWeight: 700, fontFamily: "'JetBrains Mono', monospace", color: fpsStatus === 'smooth' ? '#2ed573' : fpsStatus === 'moderate' ? '#feca57' : '#ff6b6b' }}>
              {fps !== undefined ? fps : 60} {t('telemetry.fps')}
            </div>
            <span style={{ fontSize: '11px', fontFamily: "'JetBrains Mono', monospace", color: colors.textMuted }}>{frameTimeMs !== undefined ? frameTimeMs.toFixed(1) : '16.7'} ms</span>
          </div>
          <div style={{ fontSize: '11px', marginTop: '4px', fontWeight: 500, color: fpsStatus === 'smooth' ? '#2ed573' : fpsStatus === 'moderate' ? '#feca57' : '#ff6b6b' }}>
            {fpsStatus === 'smooth' ? t('telemetry.statusSmooth') : fpsStatus === 'moderate' ? t('telemetry.statusModerate') : t('telemetry.statusLag')}
          </div>
        </div>
      </div>
    </div>
